--CREATE VIEW JC_BOQ2 AS
--ALTER VIEW JC_BOQ2 AS
...
FROM
	...
	---------------------------------
	(SELECT
		(SELECT TOP 1 B1.DocEntry FROM [@BOQ2] B1 
	...
	INNER JOIN [@BOQ2] R2 ON R1.U_BaseEntry = R2.U_BaseEntry
		AND R1.U_BaseLine = R2.U_BaseLine AND R1.DocEntry = R2.DocEntry
	---------------------------------
	--Original--
	---------------------------------
	RIGHT JOIN [@BOQ2] O1 ON O1.DocEntry = R1.U_BaseEntry AND O1.LineId = R1.U_BaseLine
	---------------------------------
